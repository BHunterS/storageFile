        ...
        <div className="mt-2 space-y-1">
            {criteria.map((item) => (
                <div key={item.label} className="flex items-center text-xs">
                    {item.met ? (
                        <Check className="size-4 text-brand-500 mr-2" />
                    ) : (
                        <X className="size-4 text-brand-500 mr-2" />
                    )}
                    <span className={item.met ? "text-green" : "text-red"}>
                        {item.label}
                    </span>
                </div>
            ))}
...
// Типи для PasswordStrengthMeter
    ...
    return (
        <div className="mt-2">
            ...
            <div className="flex space-x-1">
                ...
                ${index < strength ? getColor(strength) : "bg-brand-700"}
              `}
                    />
                ))}
            </div>
